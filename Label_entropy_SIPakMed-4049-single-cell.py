# SipakMedの分析　4049 single-cell ラベルエントロピー（本実験）
...
# JSONファイル(4049個のnodeをKey（ファイル名）-Value（特徴ベクトル）で格納)を読み込む
...
# ノードラベルのリストを作成
### 参考：仮に950 の場合　上から順にME(271),KO(232),DY(223),PA(108),SU(116)の計950
# 上から順に,DY(813) KO(825),ME(793),PA(787),SU(831)の計4049
labels = ['DY'] * 813 + ['KO'] * 825 + ['ME'] * 793 + ['PA'] * 787 + ['SU'] * 831
...
for i, node in enumerate(G.nodes()):
    ...
    # 上から順に,DY(813) KO(825),ME(793),PA(787),SU(831)の計4049
    predicted_label = ['DY', 'KO', 'ME', 'PA', 'SU'][predicted_label_index]
...
for _ in range(num_trials):
 # ★SG値。例えば　> 0.7 ということは全体の3割をゼロとして、7割をそのまま初期データとして残すということ
