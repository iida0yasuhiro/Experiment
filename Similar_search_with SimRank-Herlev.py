...
# NNDescentオブジェクトの作成
index = pnd.NNDescent(feature_vectors)

# 最近傍探索
indices, distances = index.query(feature_vectors, k=5)
...
def weighted_simrank(G, c=0.6, max_iter=100, eps=1e-6):
...
for user_input in range(len(nodes)):
    ...
    # 上位2件のラベルを取得
    #hausdorff_labels = [G.nodes[nodes[node_idx]]['label'] for node_idx, _ in hausdorff_sim_scores[1:5]]
    
    # 同じラベルの数をカウント
    #same_label_count_hausdorff = hausdorff_labels.count(node_label)
    #total_cosine_same_labels += same_label_count_hausdorff

print(f"SimRank: 同じラベルの合計数 {(total_simrank_same_labels/1834)}, Cosine Similarity: 同じラベルの合計数 {(total_cosine_same_labels/1834)}")